...
export const Home: React.FC<HomeProps> = ({ city, onSearch, onCategorySelect, onListingClick, onSelectCity }) => {
  ...
  useEffect(() => {
    ...
    // Modified: Now passing city.id to filter banners locally for that city
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (val.length > 1) setShowSuggestions(true); else setShowSuggestions(false);
  };

  const handleSubmit = (e?: React.FormEvent) => { if (e) e.preventDefault(); onSearch(query); setShowSuggestions(false); };
  ...
  return (
    <div className="flex flex-col items-center px-4 pt-12 md:pt-32 pb-20 overflow-x-hidden">
      <div className="w-full max-w-2xl text-center space-y-6">
        <div className="flex flex-col items-center space-y-4 mb-8">
          {config.logoUrl ? <img src={config.logoUrl} className="h-24 md:h-40 lg:h-48 w-auto object-contain animate-in fade-in zoom-in-95 duration-700" alt={config.siteName} /> : <h1 className="text-7xl md:text-9xl font-black text-gray-900 tracking-tighter">{config.siteName}</h1>}
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.4em]">Enterprise Local Marketplace</p>
        </div>
        
        <div className="relative group">
          <form onSubmit={handleSubmit} className="flex items-center bg-white border border-gray-200 rounded-[2rem] shadow-2xl transition-all p-1.5 overflow-hidden z-20 relative">
            <i className="fas fa-search text-gray-400 ml-6 mr-4 text-xl"></i>
            <input type="text" placeholder={`Find anything in ${city.name}...`} className="flex-1 py-4 text-lg outline-none bg-transparent placeholder:text-gray-300 font-medium" value={query} onChange={handleInputChange} />
            <button type="submit" className="bg-blue-600 text-white px-10 py-4 rounded-3xl font-black uppercase tracking-widest text-xs hover:bg-blue-700 shadow-lg hidden sm:block">Search</button>
          </form>
        </div>

        <div className="flex flex-wrap justify-center gap-2 mt-8">
          {categories.map(cat => (
            <button key={cat.id} onClick={() => onCategorySelect(cat.name)} className="px-5 py-2.5 bg-white border border-gray-100 hover:border-blue-200 hover:text-blue-600 text-gray-500 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all shadow-sm flex items-center gap-2"><i className={`fas ${cat.icon}`}></i>{cat.name}</button>
          ))}
        </div>
      </div>

      {/* ENHANCED: City-Locked Banner Carousel */}
      {banners.length > 0 && (
        <div className="w-full max-w-7xl mt-16 px-4 relative group">
          ...
          {listings.slice(0, 8).map(l => (
            <div key={l.id} onClick={() => onListingClick(l)} className="bg-white rounded-[2rem] overflow-hidden shadow-sm hover:shadow-2xl border border-gray-100 transition-all cursor-pointer group">
              <div className="relative aspect-[4/3] overflow-hidden">
                ...
                {l.isPremium && <div className="absolute top-4 left-4 bg-yellow-400 text-yellow-900 text-[9px] font-black uppercase px-2 py-1 rounded shadow-lg border border-yellow-500/20"><i className="fas fa-crown"></i> Premium</div>}
              </div>
              <div className="p-6">
